...
NamedOutputs slice(const NodeContext& node) {
    ...
    std::shared_ptr<ngraph::Node> result = std::make_shared<StridedSlice>(data,
                                                                          fixed_start_node,
                                                                          fixed_end_node,
                                                                          std::vector<int64_t>{0},
                                                                          std::vector<int64_t>{0});
    // deal with hidden attribute 'decrease_axis'
    auto decrease_axis = node.get_attribute<std::vector<int32_t>>("decrease_axis");
    if (!decrease_axis.empty()) {
        // source code shows the decreased axis must have size 1:
        //  https://github.com/PaddlePaddle/Paddle/blob/aefec228628ddf613d2006c055c57c7aad004adb/paddle/fluid/operators/strided_slice_op.cc#L150
        result =
            std::make_shared<Squeeze>(result, Constant::create(element::i32, {decrease_axis.size()}, decrease_axis));
    }

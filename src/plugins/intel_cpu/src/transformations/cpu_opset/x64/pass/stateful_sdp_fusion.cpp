...
StatefulSDPFusion::StatefulSDPFusion() {
    ...
    ov::matcher_pass_callback callback = [=](Matcher& m) {
        ...
        // const auto assign_k_node = ov::as_type_ptr<opset6::Concat>(pattern_map.at(assign_k).get_node_shared_ptr());
        // const auto assign_v_node = ov::as_type_ptr<opset6::Concat>(pattern_map.at(assign_v).get_node_shared_ptr());
        auto* assign_k_node = find_assign(pattern_map.at(concat_k_node));
        if (!assign_k_node)
            return false;
        if (past_k_node->get_variable_id() != assign_k_node->get_variable_id())
            return false;

        auto* assign_v_node = find_assign(pattern_map.at(concat_v_node));
        if (!assign_v_node)
            return false;

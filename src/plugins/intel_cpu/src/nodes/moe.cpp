...
struct MOE::Executor : MOE::ExecutorBase {
    ...
    void execute(const dnnl::stream& astream, MOE* pnode) override {
        ...
        memset(final_hidden_states, 0, n_states * n_tokens * output_precisions[0].bitwidth() / 8);
                ...
                parallel_for(n_tokens, [&](int i) {
                    ...
                    for (size_t k = 0; k < pnode->m_config.topk; k++) {
                        if (p_expert_ids[k] == expert_id) {
                            ...
                            memcpy(dst, hidden_states + (i * n_states * act_dtype_size), n_states * act_dtype_size);
                            // gather weights

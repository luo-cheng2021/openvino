...
Convolution::Convolution(const std::shared_ptr<ngraph::Node>& op, const dnnl::engine& eng, WeightsSharing::Ptr &cache)
    ...
    //   static shape(dynamic shape may change weights layout if the input shape changes and cause performance issue: 86948)
    shouldTryBrgconv = !isDynamicNode();
    char* p = getenv("USE_BRG");
    shouldTryBrgconv = p && p[0] == '1';
}
